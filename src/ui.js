...
function supplyNew(){
  ...
  setMain(topbar("Aprovizionare nouă", '<button class="ghost" onclick="pgo(\\'supply\\')">← Înapoi</button>')
    ...
    + '<div class="card" style="padding:20px"><h2>Produse trimise</h2>'
      ...
      + '<div class="fhint" style="margin:10px 0 4px">sau produs nou (dacă nu e încă în sistem). <b>Codul de bare se atribuie la recepție, în depozit</b> — îl lași gol dacă nu-l ai.</div>'
      ...
      + '<div style="width:120px"><label>Cod EAN (dacă îl ai)</label><input id="sup_nbc" placeholder="opțional"></div>'
...
VIEWS.dashboard = function(){
  ...
  api("GET","/api/dashboard").then(function(d){
    ...
    var quick = '<div class="card" style="padding:16px;margin-bottom:16px"><h2 style="margin:0 0 12px">Acces rapid</h2><div class="dgrid">'
      ...
      + dashTile("departures","🚚","Marfa a plecat","operator")
...
// Pagină dedicată „Marfa a plecat" (din Acces rapid) — toate comenzile pregătite + buton de plecare.
VIEWS.departures = function(){
  setMain(topbar("Marfa a plecat") + '<div class="card" id="dep_view">Se încarcă…</div>');
  loadDepartures();
};
function loadDepartures(){
  var host=el("dep_view"); if(!host) return;
  api("GET","/api/orders?type=outbound&status=prepared").then(function(d){
    var list=d.orders||[];
    if(!list.length){ host.innerHTML='<div class="muted center" style="padding:24px">Nicio comandă pregătită de plecare.<br><span style="font-size:12.5px">Comenzile apar aici după picking (când marfa e pregătită și rezervată).</span></div>'; return; }
    var rows=list.map(function(o){
      var who = o.source==="portal"
        ? esc(o.client_name||"client")+(o.recipient_name?(' <span class="muted">→ '+esc(o.recipient_name)+'</span>'):'')
        : esc(o.partner_name||"—");
      return '<tr><td><b>'+esc(o.code)+'</b></td><td>'+who+'</td>'
        ...
        +'<td class="right"><button class="sm" onclick="departView('+o.id+',\\''+o.code+'\\')">🚚 Marfa a plecat</button>'
        +' <button class="ghost sm" onclick="orderDetail('+o.id+')">Vezi</button></td></tr>';
    }).join("");
    host.innerHTML='<div class="muted" style="font-size:12.5px;margin-bottom:10px">Comenzi pregătite (stoc rezervat). Când confirmi plecarea, stocul scade definitiv și comanda se finalizează.</div>'
      +'<table><thead><tr><th>Cod</th><th>Client</th><th class="right">Cant.</th><th>Status</th><th></th></tr></thead><tbody>'+rows+'</tbody></table>';
  }).catch(function(e){ host.innerHTML='<div class="pill bad">'+esc(e.message)+'</div>'; });
}
window.departView = function(id, code){
  modal("Marfa a plecat — "+esc(code||("#"+id)),
    '<p>Confirmi că marfa comenzii <b>'+esc(code||("#"+id))+'</b> a plecat din depozit?</p>'
    +'<p class="muted" style="font-size:13px">Stocul rezervat se scade acum definitiv, iar comanda devine «finalizată».</p>',
    function(){
      api("POST","/api/orders/"+id+"/depart",{}).then(function(){ closeModal(); toast("Marfa a plecat — stoc actualizat"); loadDepartures(); }).catch(function(e){ toast(e.message,"bad"); });
    });
  var sv=el("modalSave"); if(sv){ sv.textContent="Da, a plecat"; sv.className=""; sv.style.display=""; }
};
...
function opForm(title, type){
  var avizBtn = type==="receive" ? '<button class="ghost" onclick="palletReceiveUI(\\'palet\\')">📦 Recepție palet</button> <button class="ghost" onclick="palletReceiveUI(\\'colet\\')">📥 Recepție colet</button> <button class="ghost" onclick="palletReceiveUI(\\'ambalaje\\')">🎁 Palet ambalaje</button> <button class="ghost" onclick="xlsInUI()">📊 Recepție Excel</button> <button class="ghost" onclick="avizUI()">📄 Aviz PDF</button>'
...
VIEWS.backup = function(){
  setMain(topbar("Backup & Restaurare")
    ...
    +     '<option value="1">La fiecare oră</option>'
    +     '<option value="6">La 6 ore</option>'
    +     '<option value="12">La 12 ore</option>'
    +     '<option value="24">Zilnic</option>'
    +     '<option value="168">Săptămânal</option>'
    +   '</select></div>'
    +   '<div id="bk_status" class="muted" style="font-size:12px"></div>'
    + '</div>'
...
/* ---- Recepție pe palet / colet / palet ambalaje (creează unitatea + produse + stoc + aviz + etichetă) ---- */
function palKind(k){ return k==="colet"?"Colet":(k==="ambalaje"?"Palet ambalaje":"Palet"); }
var _palRec = { lines: [], products: [], kind: "palet", avizFile: null };
window.palletReceiveUI = function(kind){
  kind = (kind==="colet"||kind==="ambalaje") ? kind : "palet";
  _palRec = { lines: [], products: [], kind: kind, avizFile: null };
  var isColet = kind==="colet", isPal = !isColet;
  var kindLbl = palKind(kind), kindLc = kindLbl.toLowerCase();
  var pref = kind==="colet"?"COL":(kind==="ambalaje"?"AMB":"PAL");
  var today = new Date().toISOString().slice(0,10);
  var coleteField = isPal
    ...
    : '';
  setMain(topbar("Recepție "+kindLc, '<button class="ghost" onclick="go(\\'receive\\')">← Recepție</button>')
    ...
    + '<div class="row"><div style="flex:1"><label>Cod '+kindLc+' (opțional)</label><input id="pal_code" placeholder="auto: '+pref+'-xxxxx"></div>'
    ...
    + '<button class="big" style="margin-top:16px" onclick="palReceiveSubmit()">Recepționează '+kindLc+'</button></div>');
...
window.palrAddLine = function(){
  var pid=el("pal_prod")?Number(el("pal_prod").value):0;
  var boxes=el("pal_boxes")?Number(el("pal_boxes").value):0;
  var per=el("pal_perbox")?Number(el("pal_perbox").value):0;
  if(!pid){ toast("Alege un produs","bad"); return; }
  if(!(boxes>0)||!(per>0)){ toast("Completează nr. cutii și buc/cutie","bad"); return; }
  var p=(_palRec.products||[]).find(function(x){return x.id===pid;});
  var ex=_palRec.lines.find(function(l){return l.product_id===pid;});
  if(ex){ ex.boxes=(ex.boxes||0)+boxes; ex.per_box=per; ex.quantity=ex.boxes*ex.per_box; }
  else _palRec.lines.push({product_id:pid, boxes:boxes, per_box:per, quantity:boxes*per, name:(p?p.name:("#"+pid)), code:(p?(p.barcode||p.sku):"")});
  if(el("pal_boxes")) el("pal_boxes").value=1;
  palrRenderLines();
};
...
function palrRenderLines(){
  ...
  var tot=_palRec.lines.reduce(function(a,l){return a+(l.quantity||0);},0);
  host.innerHTML='<table><thead><tr><th>Produs</th><th class="right">Cutii × buc</th><th class="right">Total</th><th></th></tr></thead><tbody>'
    +_palRec.lines.map(function(l){ return '<tr><td>'+esc(l.name)+' <span class="muted" style="font-size:11.5px">'+esc(l.code)+'</span></td><td class="right">'+(l.boxes&&l.per_box?(l.boxes+' × '+l.per_box):'—')+'</td><td class="right"><b>'+l.quantity+'</b></td><td class="right"><button class="danger sm" onclick="palrDelLine('+l.product_id+')">✕</button></td></tr>'; }).join("")
    +'<tr><td colspan="2" class="right"><b>Total bucăți</b></td><td class="right"><b>'+tot+'</b></td><td></td></tr>'
    +'</tbody></table>';
}
window.palReceiveSubmit = function(){
  ...
  var body={ kind:_palRec.kind, location_id:loc,
    ...
    items:_palRec.lines.map(function(l){return {product_id:l.product_id, boxes:l.boxes, per_box:l.per_box, quantity:l.quantity};}) };
  api("POST","/api/pallets/receive",body).then(function(r){
    toast(palKind(_palRec.kind)+" recepționat: "+r.pallet.code);
...
window.leApplyPreset = function(idx){
  ...
  _le.tpl={ width_mm:_le.tpl.width_mm||100, height_mm:_le.tpl.height_mm||150, valign:_le.tpl.valign||"center", orient:_le.tpl.orient||"portrait", elements:JSON.parse(JSON.stringify(p.elements)) };
...
VIEWS.labeleditor = function(){
  ...
  setMain(topbar("Editor etichetă")
    ...
    + '</select></div>'
    + '<div class="field" style="width:140px;margin:0"><label>Orientare</label><select id="le_orient" onchange="leSetDim(\\'orient\\',this.value)">'
    + [["portrait","Portret"],["landscape","Landscape"]].map(function(a){return '<option value="'+a[0]+'"'+((_le.tpl.orient||"portrait")===a[0]?' selected':'')+'>'+a[1]+'</option>';}).join("")
    + '</select></div>'
    ...
    + '<div class="muted" style="font-size:11.5px;margin-top:6px">Aproximativ — pe imprimanta Zebra iese centrat exact.</div></div>'
    + '</div>'
    + '<div class="row" style="margin-top:14px"><button onclick="leSave()">Salvează</button> <button class="ghost sm" onclick="leResetTpl()">Revino la implicit</button></div>'
    + '</div>');
  leLoad();
};
window.leSetType = function(t){ _le.type=t; go("labeleditor"); };
function leLoad(){
  api("GET","/api/print/label-template?type="+_le.type).then(function(r){
    _le.fields=r.fields||[]; _le.tpl=(r.template&&Array.isArray(r.template.elements))?r.template:{elements:[]};
    leRenderRows(); lePreview();
  }).catch(function(e){ toast(e.message,"bad"); });
}
function leCanBc(field){ var f=_le.fields.find(function(x){return x[0]===field;}); return !!(f&&f[2]); }
function leFieldLabel(field){ var f=_le.fields.find(function(x){return x[0]===field;}); return f?f[1]:field; }
function leRenderRows(){
  var host=el("le_rows"); if(!host) return;
  if(!_le.tpl.elements.length){ host.innerHTML='<div class="muted" style="font-size:12.5px;padding:8px 0">Niciun rând. Apasă „+ Adaugă rând".</div>'; return; }
  host.innerHTML=_le.tpl.elements.map(function(e,i){
    var canBc=leCanBc(e.field);
    var fopts=_le.fields.map(function(f){return '<option value="'+f[0]+'"'+(f[0]===e.field?' selected':'')+'>'+esc(f[1])+'</option>';}).join("");
    var isBc=(e.render==="barcode"&&canBc);
    var renderSel = canBc ? ('<select onchange="leSet('+i+',\\'render\\',this.value)"><option value="text"'+(!isBc?' selected':'')+'>Text</option><option value="barcode"'+(isBc?' selected':'')+'>Cod de bare</option></select>') : '<span class="muted" style="font-size:12px">text</span>';
    var sizes = isBc ? [["sm","Mic"],["md","Mediu"],["lg","Mare"]] : [["sm","Mic"],["md","Mediu"],["lg","Mare"],["xl","F. mare"]];
    var sizeSel='<select onchange="leSet('+i+',\\'size\\',this.value)">'+sizes.map(function(s){return '<option value="'+s[0]+'"'+(e.size===s[0]?' selected':'')+'>'+s[1]+'</option>';}).join("")+'</select>';
    var alignSel='<select onchange="leSet('+i+',\\'align\\',this.value)">'+[["L","Stânga"],["C","Centru"],["R","Dreapta"]].map(function(a){return '<option value="'+a[0]+'"'+(e.align===a[0]?' selected':'')+'>'+a[1]+'</option>';}).join("")+'</select>';
    var fixedInput = e.field==="fixed" ? ('<input placeholder="text fix" value="'+esc(e.text||"")+'" oninput="leSet('+i+',\\'text\\',this.value)" style="margin-top:6px;width:100%">') : '';
    return '<div class="card" style="padding:8px;margin-bottom:6px;background:var(--panel-2)">'
      +'<div class="row" style="gap:6px;flex-wrap:wrap;align-items:center">'
      +'<select onchange="leSet('+i+',\\'field\\',this.value)" style="flex:2;min-width:130px">'+fopts+'</select>'
      +renderSel+sizeSel+alignSel
      +'<span style="flex:1"></span>'
      +'<button class="ghost sm" onclick="leMove('+i+',-1)" title="Sus">▲</button>'
      +'<button class="ghost sm" onclick="leMove('+i+',1)" title="Jos">▼</button>'
      +'<button class="danger sm" onclick="leDel('+i+')">✕</button>'
      +'</div>'+fixedInput+'</div>';
  }).join("");
}
window.leSetDim = function(key,val){
  if(key==="valign"||key==="orient"){ _le.tpl[key]=val; }
...
window.leSet = function(i,key,val){
  ...
  e[key]=val;
  if(key==="field"){ if(!leCanBc(val)) e.render="text"; leRenderRows(); }
  else if(key==="render"){ leRenderRows(); }
  lePreview();
};
window.leAdd = function(){
  var first=_le.fields[0]?_le.fields[0][0]:"fixed";
  _le.tpl.elements.push({field:first, render:"text", size:"md", align:"C", text:""});
  leRenderRows(); lePreview();
};
window.leDel = function(i){ _le.tpl.elements.splice(i,1); leRenderRows(); lePreview(); };
window.leMove = function(i,d){ var a=_le.tpl.elements; var j=i+d; if(j<0||j>=a.length) return; var t=a[i]; a[i]=a[j]; a[j]=t; leRenderRows(); lePreview(); };
function leSample(type){
  var d=new Date().toLocaleString();
  if(type==="pallet") return {kind:"PALET",code:"PAL-00001",client:"ACME SRL",location:"A1",lot:"LOT 1",aviz:"AV123",date:d,items:"Produs A  10\\nProdus B  5"};
  if(type==="box") return {box_code:"CUT-00001",product_barcode:"2000000002088",product_name:"CUTIA BEBELUSULUI",lot:"LOT 1",qty:"12",date:d,meta:"Lot: LOT 1   "+d};
  return {title:"CUTIA BEBELUSULUI",code:"2000000002088",lot:"LOT 1",date:d,meta:"Lot: LOT 1   "+d};
}
function lePreview(){
  ...
  var landscape=(_le.tpl.orient==="landscape");
  // cadrul de design (upright) desenat offscreen, apoi rotit la blit pentru landscape
  var offW = landscape ? Math.round(cw*(wmm/hmm)) : cw;
  var offH = Math.round(offW*(hmm/wmm));
  var visW = cw, visH = landscape ? Math.round(cw*(wmm/hmm)) : offH;
  var scale=offW/(wmm/25.4*300);
  ...
  var valign=_le.tpl.valign||"center", y0=pad, extra=0;
  if(valign==="center") y0=Math.max(pad,(offH-contentH)/2);
  else if(valign==="spread"&&blocks.length>1) extra=Math.max(0,(offH-contentH-2*pad)/(blocks.length-1));
  function ax(al,wpx){ if(al==="L")return pad; if(al==="R")return offW-pad-wpx; return (offW-wpx)/2; }
  // offscreen (design upright)
  var off=document.createElement("canvas"); off.width=offW; off.height=offH;
  var octx=off.getContext("2d"); octx.fillStyle="#fff"; octx.fillRect(0,0,offW,offH); octx.fillStyle="#000";
  // preload barcode images
  var imgs=[]; blocks.forEach(function(b){ if(b.t==="bc"){ var im=new Image(); im.src=lblBarcodeURL(b.val); b._img=im; imgs.push(im); } });
  function drawAll(){
    var yy=y0;
    blocks.forEach(function(b){
      if(b.t==="tx"){ octx.font="bold "+Math.max(7,b.fs)+"px Arial"; octx.textBaseline="top";
        var lines=b.field==="items"?b.val.split("\\n"):[b.val]; var ly=yy;
        lines.forEach(function(ln){ var wpx=octx.measureText(ln).width; octx.fillText(ln, ax(b.align,wpx), ly); ly+=b.fs+2; });
      } else if(b._img&&b._img.complete&&b._img.width){
        var wpx=Math.min(offW-2*pad,b._img.width), hpx=b.bh, x=ax(b.align,wpx); octx.drawImage(b._img,x,yy,wpx,hpx);
        octx.font=Math.max(7,11*scale)+"px Arial"; octx.textBaseline="top"; var tw=octx.measureText(b.val).width; octx.fillText(b.val, ax(b.align,tw), yy+hpx+1);
      }
      yy+=b.h+b.gap+extra;
    });
    // blit pe canvas vizibil
    c.width=visW; c.height=visH;
    var ctx=c.getContext("2d"); ctx.fillStyle="#fff"; ctx.fillRect(0,0,visW,visH);
    if(!landscape){ ctx.drawImage(off,0,0,visW,visH); }
    else { ctx.save(); ctx.translate(visW,0); ctx.rotate(Math.PI/2); ctx.drawImage(off,0,0,offW,offH,0,0,visH,visW); ctx.restore(); }
  }
  var pending=imgs.filter(function(im){return !im.complete;}).length;
  if(!pending){ drawAll(); }
  else { imgs.forEach(function(im){ if(im.complete) return; im.onload=function(){ pending--; if(pending<=0) drawAll(); }; im.onerror=function(){ pending--; if(pending<=0) drawAll(); }; }); drawAll(); }
}
window.leSave = function(){
  api("PUT","/api/print/label-template",{type:_le.type, template:{width_mm:_le.tpl.width_mm, height_mm:_le.tpl.height_mm, valign:_le.tpl.valign, orient:_le.tpl.orient, elements:_le.tpl.elements}}).then(function(){ toast("Șablon salvat pentru "+_le.type); }).catch(function(e){ toast(e.message,"bad"); });
};
window.leResetTpl = function(){
  modal("Revino la implicit","<p>Sigur revii la eticheta implicită pentru acest tip? Modificările tale se pierd.</p>", function(){
    api("POST","/api/print/label-template/reset",{type:_le.type}).then(function(r){ _le.tpl=r.template||{elements:[]}; closeModal(); leRenderRows(); lePreview(); toast("Revenit la implicit"); }).catch(function(e){ toast(e.message,"bad"); });
  });
  var sv=el("modalSave"); if(sv){ sv.textContent="Da, revino"; sv.className="danger"; sv.style.display=""; }
};
...
function boxLabelCss(){
  return '@page{size:auto;margin:0}html,body{margin:0}body{font-family:Arial,Helvetica,sans-serif;color:#000}'
    ...
    +'.meta{font-size:12px;color:#333;margin-top:6px}img{max-width:100%;max-height:90px}';
}
...
function palShipRender(p, items){
  ...
  var meta='<div class="muted" style="font-size:13px;line-height:1.7;margin-top:4px">'
    +'Tip: <b>'+palKind(p.kind)+'</b> · Client: <b>'+esc(p.client_name||"—")+'</b> · Locație: <b>'+esc(p.location_code||"—")+'</b><br>'
...
window.palletDetail = function(id){
  api("GET","/api/pallets/"+id).then(function(d){
    ...
    var items='<table><thead><tr><th>SKU</th><th>Produs</th><th class="right">Cant.</th>'+(can("operator")?'<th></th>':'')+'</tr></thead><tbody>'
      + d.items.map(function(it){ var bx=(it.boxes&&it.per_box)?('<span class="muted" style="font-size:11.5px"> ('+it.boxes+'×'+it.per_box+')</span>'):''; return '<tr><td><b>'+esc(it.sku)+'</b></td><td>'+esc(it.product_name)+'</td><td class="right">'+esc(it.quantity)+bx+'</td>'+(can("operator")?'<td class="right"><button class="ghost sm" onclick="palDelItem('+id+','+it.id+')">✕</button></td>':'')+'</tr>'; }).join("")
      + '</tbody></table>';
    var actions='';
    if(can("operator")){
    ...
    var title = palKind(p.kind)+" "+esc(p.code)+(p.location_code?(" · "+esc(p.location_code)):" · neplasat");
...
function bkFmtStatus(d){
  ...
  } else { parts.push('<span class="muted">încă nerulat</span>'); }
  if(d.last_status==="fail" && d.last_error) parts.push('<div style="font-size:11px;margin-top:3px" class="muted">'+esc(d.last_error)+'</div>');
  return parts.join(" ");
}
window.loadBackupSettings = function(){
  api("GET","/api/admin/backup-settings").then(function(d){
    if(el("bk_interval")) el("bk_interval").value=String(d.interval_hours||0);
...
window.saveBackupSettings = function(){
  ...
  api("PUT","/api/admin/backup-settings",{interval_hours:h}).then(function(){
    toast(h>0?("Backup automat setat: "+(h===1?"la fiecare oră":h===24?"zilnic":h===168?"săptămânal":("la "+h+" ore"))):"Backup automat oprit");
  }).catch(function(e){ toast(e.message,"bad"); });
};
