...
export async function list(request, env) {
  ...
  const binds = [];
  if (q) {
    sql += ' WHERE (sku LIKE ? OR name LIKE ? OR barcode LIKE ?)';
    const like = `%${q}%`;
...
export async function create(request, env) {
  ...
  try {
    const res = await env.DB.prepare(
      ...
      b.category || null, b.unit || 'buc', Number(b.reorder_point) || 0
    ).run();
...
export async function update(request, env, ctx, user, params) {
  ...
  await env.DB.prepare(
    `UPDATE products SET sku=?, barcode=?, name=?, description=?, category=?, unit=?, reorder_point=?, active=? WHERE id=?`
  ).bind(
    m.sku, m.barcode || null, m.name, m.description || null, m.category || null,
    m.unit || 'buc', Number(m.reorder_point) || 0, m.active ? 1 : 0, id
  ).run();
